...
1. **装好两个技能**：
   - **本仓库（`老登股推荐`）**：直接克隆本仓库即可，克隆下来的文件夹就是一个完整的技能。
   - **配套筛选器（`ashare-graham-screener`）**：用于 Graham 七条件选股，是独立技能。放到与本级技能同一目录下即可（两个文件夹并列）。
2. **直接跟 AI 说一句话**，例如：
...
- **只想分析别人给的选股结果？** 如果手头已经有一个 `A股防御型选股_*.md` 文件，跑一条命令即可（见第四节、第八节）：
  ...
  ```
- **只有本仓库、没有筛选器？** 本仓库只含「分析 + 报告」；筛选那一步需要配套的 `ashare-graham-screener` 技能。两个都装上就能一键全流程跑；或者你自己准备一份选股 `.md` 文件喂进来也行（跳过筛选步骤，直接分析）。

> 一句话总结：**克隆本仓库 + 装好筛选器 → 跟 AI 说一句话 → 拿 HTML 报告**。就这么简单。
...
3. 已有 **Graham 选股结果 Markdown**（由 `ashare-graham-screener` 或手动准备）
