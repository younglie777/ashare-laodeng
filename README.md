...
- [ ] **2. 准备 `westock-data` 与 `westock-tool` 数据组件**（仅 `fetch` / `all` 全自动模式需要）：本技能靠它们拉 A 股行情/财务。
  - **WorkBuddy 用户**：在技能面板搜索并「启用」内置的 `westock-data`、`westock-tool` 即可。
  - **其他 Agent / 环境**：用 `npm` 安装（或克隆其仓库）`westock-data` 与 `westock-tool`，把各自的 `scripts/index.js` 路径分别设到环境变量 `WESTOCK_DATA`、`WESTOCK_TOOL`（或加入 PATH）。脚本运行时会自动探测。
  - 没准备好 → `all`/`fetch` 会提示"未找到 westock"并给出安装指引。只做 `analyze`（分析已有 md）则不需要。
...
> 🤖 **跨 Agent 依赖自检**：本技能的 `run_pipeline.py` 在运行时（`fetch`/`screen`/`all`）会**自动检测 Node.js 与 westock 组件是否就绪**。缺失时会在输出末尾打印一行 `>>> PREFLEFT: fail` 并列出「缺什么 + 怎么装」（WorkBuddy 走面板启用；其他环境走 npm 或 `WESTOCK_DATA`/`WESTOCK_TOOL` 环境变量）。**任何 Agent 读到这行即可把缺失项与安装步骤转达给用户**，补齐后重跑同一条命令即继续。

**非 WorkBuddy 环境如何准备 westock（用于 `fetch`/`all` 全自动拉数据）**：

- `npm install -g westock-data westock-tool`（或克隆对应仓库），再把各自的 `scripts/index.js` 路径设到环境变量 `WESTOCK_DATA`、`WESTOCK_TOOL`；脚本会自动探测，无需改代码。
- 若只做「分析已有选股 md → 出报告」（`analyze` 子命令），**完全不需要 westock / Node**，纯 Python 标准库即可，在任何 Agent 环境都能直接跑。
