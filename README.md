...
1. **装好两个技能**：把本技能 `老登股推荐` 和配套筛选器 `ashare-graham-screener` 都放进 AI 的技能目录（WorkBuddy 下是 `~/.workbuddy/skills/`，两个并列放）。
2. **直接跟 AI 说一句话**，例如：
...
- **只想分析别人给的选股结果？** 如果手头已经有一个 `A股防御型选股_*.md` 文件，跑一条命令即可（见第四节、第八节）：
  ...
  ```
- **只有本仓库、没有筛选器？** 本仓库只含「分析 + 报告」；筛选那一步需要配套的 `ashare-graham-screener`。把那个技能也一起放进来，全流程就能一键跑；或者你自己准备一份选股 `.md` 喂进来也行。

> 一句话总结：**装两个技能 → 跟 AI 说一句话 → 拿 HTML 报告**。就这么简单。
